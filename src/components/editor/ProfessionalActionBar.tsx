...
import { 
  ...
  ExternalLink, Loader2, Settings, Info
} from 'lucide-react'
...
export default function ProfessionalActionBar({
  ...
  const renderGenerateButton = () => (
    <div className="flex-1 max-w-md">
      ...
      {!canGenerate && (
        <p className="text-xs text-[#ADADAD] mt-1 text-center">
          {!isConnected ? 'Connect wallet to generate' : 'Complete required fields'}
        </p>
      )}
    </div>
  )
  ...
  const renderStatus = () => {
    ...
    return (
      <div className={cn(
        ...
        <div className="flex items-center gap-2">
          {mintStatus === 'pending' && <Clock className="w-4 h-4 text-yellow-400 animate-pulse" />}
      ...
      <div className="flex items-center gap-3">
        <div className="flex-1">
          <div className="flex items-center justify-between mb-1">
            <span className="text-xs text-[#ADADAD]">{progressText || 'Processing...'}</span>
            {progress && (
              <span className="text-xs text-[#ADADAD]">{Math.round(progress)}%</span>
            )}
          </div>
          <Progress 
            value={progress || 0} 
  ...
  return (
    <div className="space-y-1">
      ...
      {/* Connection Status */}
      {!isConnected && (
        <div className="flex items-center justify-center gap-2 p-2 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
