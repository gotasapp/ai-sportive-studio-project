...
export default function FeaturedCarousel() {
  ...
  if (loading) {
    return (
      <div className="w-full h-[450px] bg-card rounded-2xl flex flex-col items-center justify-center">
  ...
  if (featuredNFTs.length === 0) {
    return (
      <div className="w-full h-[450px] bg-card rounded-2xl flex flex-col items-center justify-center text-center p-8">
  ...
  return (
    <div className="relative w-full h-[450px] rounded-2xl overflow-hidden group">
      ...
      <div className="absolute top-4 left-4">
        <div className="bg-accent/90 backdrop-blur-sm px-3 py-1 rounded-full flex items-center space-x-2">
      ...
      {featuredNFTs.length > 1 && (
        <>
          <button 
            ...
            className="absolute left-4 top-1/2 -translate-y-1/2 bg-black/50 backdrop-blur-sm p-3 rounded-full text-white opacity-0 group-hover:opacity-100 transition-all duration-300 hover:bg-black/70"
          >
            <ChevronLeft size={24} />
          </button>
          <button 
            onClick={goToNext} 
            className="absolute right-4 top-1/2 -translate-y-1/2 bg-black/50 backdrop-blur-sm p-3 rounded-full text-white opacity-0 group-hover:opacity-100 transition-all duration-300 hover:bg-black/70"
          >
            ...
            {featuredNFTs.map((_, index) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-all duration-300 ${
                  currentIndex === index ? 'bg-accent scale-110' : 'bg-white/50 hover:bg-white/70'
                }`}
              />
            ))}
          </div>
