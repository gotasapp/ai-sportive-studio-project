...
export default function StadiumsPage() {
  ...
  // Atualizar filteredStadiums para considerar o filtro de status
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 md:gap-0">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-200">Stadium Management</h1>
          <p className="text-gray-400 mt-2 text-sm md:text-base">Manage stadium references, base prompts, and reference images.</p>
        </div>
        <Button className="cyber-button w-full md:w-auto" onClick={refreshReferences}>
          <RefreshCw className="w-4 h-4 mr-2" />
          Refresh Data
        </Button>
      </div>
      {/* Stadium Management Section (management/referências) */}
            ...
            <TabsContent value="references" className="space-y-4 pt-4">
              {referencesLoading ? (
                ...
                <>
                  ...
                  {/* Paginação das referências */}
                  {teamReferences.length > refsPerPage && (
                    <div className="flex justify-between items-center mt-4">
                      <span className="text-gray-400 text-sm">
                        Page {currentRefPage} of {totalRefPages}
                      </span>
                      <div className="flex gap-2">
                        <Button
                          variant="outline"
                          ...
                          onClick={() => setCurrentRefPage(currentRefPage - 1)}
                        >
                          Previous
                        </Button>
                        <Button
                ...
                {paginatedImages.map(img => (
                  <Card key={img.id} className="cyber-card border-gray-700">
                    <CardContent className="p-2">
                      ...
                      <p className="text-xs font-normal text-white truncate">{img.teamName}</p>
                    </CardContent>
                  </Card>
                ))}
          </div>
            {/* Paginação das imagens */}
            {allRefImages.length > imgsPerPage && (
              <div className="flex justify-between items-center mt-4">
                <span className="text-gray-400 text-sm">
                  Page {currentImgPage} of {totalImgPages}
                </span>
                <div className="flex gap-2">
                  <Button
                    variant="outline"
                    ...
                    onClick={() => setCurrentImgPage(currentImgPage - 1)}
                  >
                    Previous
                  </Button>
                  <Button
                    variant="outline"
                    size="sm"
                    disabled={currentImgPage === totalImgPages}
                    onClick={() => setCurrentImgPage(currentImgPage + 1)}
                  >
                    Next
                  </Button>
      ...
      {/* Listagem de Stadiums Gerados com paginação */}
      <Card className="cyber-card border-cyan-500/30">
        <CardHeader>
        ...
        <CardContent>
          {stadiumsLoading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-cyan-500/20 text-cyan-300">
                    ...
                    <th className="p-3 font-semibold hidden md:table-cell">Created At</th>
                  ...
                  {paginatedStadiums.map(stadium => (
                    <React.Fragment key={stadium.id}>
                      <tr className="border-b border-gray-800 hover:bg-cyan-500/5">
                        ...
                        <td className="p-3 text-gray-400 hidden md:table-cell">{new Date(stadium.createdAt).toLocaleDateString()}</td>
                        <td className="p-3 text-right">
                          <Button variant="ghost" size="icon" className="w-10 h-10" onClick={() => setOpenRow(openRow === stadium.id ? null : stadium.id)}>
                        ...
                        <tr className="bg-gray-900/80 border-b border-gray-800">
                          <td colSpan={3} className="p-6">
                            <div className="flex flex-col md:flex-row gap-6">
                              <div className="flex-1">
                                <h4 className="text-base md:text-lg font-semibold text-cyan-400 mb-2">Stadium Details</h4>
              ...
              {/* Paginação */}
              {filteredStadiums.length > itemsPerPage && (
                <div className="flex justify-between items-center mt-4">
                  <span className="text-gray-400 text-sm">
